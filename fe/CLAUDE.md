...
- **`next/image`를 쓰지 않는다.** 최적화 대상이 우리 파일이 아니다.
  평범한 `<img loading="lazy">`를 쓴다 — `remotePatterns` 설정 문제가 아예 생기지 않는다.
  실패(404 · referer 차단)하면 자리만 비우고 카드는 그대로 선다. 레이아웃을 무너뜨리지 않는다.
  카드 1건에 **1장.** 갤러리를 만들지 않는다
- **이미지 파일을 우리 저장소에 두지 않는다** — `img src`가 원본 CDN을 그대로 가리킨다
  ([[plan]] §2.1). Shopify CDN은 `Cross-Origin-Resource-Policy` 없음 · 핫링크 차단 없음 실측 완료.
  `<img>`는 no-CORS라 CORS가 관여하지 않는다
