...
>
> 인라인 참조의 **기술적 성립 여부와 허가 상태**는 [[plan]] §2.1에 있다. 여기 복사하지 않는다.
> 요약만: 막혀 있지는 않으나 **허가는 없다.** `referrerpolicy`를 박지 않는다.
>
> **남은 미확인 1건 — 라이브 수집을 켜기 전에 반드시 닫는다:**
> `src`의 `?v=` 파라미터가 요청마다 바뀌는지. 캡처 2회 diff로 판정한다
> (`updated_at` 때와 같은 규율 — 그때는 49초 간격 2회 · 622건 전수로 잡았다).
>
> **바뀌더라도 `hash_exclude`는 대책이 아니다.** 제외는 **키 이름 기반, 깊이 무관**이라
> `src`를 넣으면 해시 입력에서 필드가 통째로 사라진다 — **진짜 이미지 교체도
> 새 mention을 못 만든다.** 의미 없는 변동만 버리는 `updated_at`과 다른 종류다.
> 바뀌는 것으로 나오면 **쿼리스트링을 떼고 정규화**하는 쪽으로 간다.
>
> 배열 순서도 같이 확인한다. `position`을 버리고 `images[0]`을 택했는데,
> 순서가 흔들리면 `?v=`와 같은 결과(무변경 재적재)가 난다 —
> `_collections`를 정렬하는 이유와 같은 함정이다 (§1 위쪽).
> `product.image`(featured)가 의미상 더 맞는 필드인지도 이때 함께 본다.
>
> 전달 경로는 **원본 CDN 인라인 참조**로 설계했다. 다만 **허가는 아직 없고**
> 회신 전에는 공개 화면에 내지 않는다 ([[plan]] §2.1 · §8.1). 이 필드는 어느 쪽이든 입력이다.
...
> 이미지 **파일 미보유**, 링크아웃, 무수익, 정지 수단.
> 화면은 「무엇을 만드는지」를 보이기 위한 것이다.
>
> **이미지는 「미호스팅」으로 뭉뚱그리지 않는다** (2026-08-30). 파일은 안 갖지만
> 공개 화면에 `<img src="원본 CDN">`으로 **표시한다.** 「링크아웃만 한다」고 적어놓고
> 인라인 표시를 하면 그게 차단 사유다 — 문의 전략 전체가 「숨기지 않는다」 위에 서 있다.
> 물어야 할 것:
> - 상품 이미지를 **원본 CDN 참조로 표시**해도 되는가. 규약의 「**공표**」에 해당하는가
> - 고정 크기 카드라 **트리밍·비율 변경**이 생긴다. 별도 허가가 필요한가
> - 참조를 그만두길 원하면 **어떤 신호로 알려줄지** (Referer 차단은 우리가 감지하지 못한다)
> - 특정 상품 이미지만 내려달라는 요청을 받을 창구
>
