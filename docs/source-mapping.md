...
```json
{
  ...
  "label_tags": ["ハチワレ", "うさぎ", "モモンガ", "ラッコ", "くりまんじゅう", "シーサー"],
  "drop_tags": ["在庫有無確認用タグ", "破棄対象商品", "同梱不可", "同梱不可A", "ラッピング不可", "キャンペーン対象外"]
}
```
...
```
1) 태그에 'ちいかわ' 또는 'ちいかわキャラクターズ' 가 있으면          → 수록
2) 컬렉션 소속에 'chiikawa' 또는 'chiikawa-characters' 가 있으면    → 수록
3) 그 외                                                          → 제외
```

> [!important] 캐릭터 태그 단독으로 판정하지 않는다
> `ハチワレ` `うさぎ`만 보고 판정하면 안 된다. 다른 작품에 같은 이름의 캐릭터가 있을 수 있고,
> 실제 관측에서 치이카와 상품에는 **`ちいかわキャラクターズ`가 항상 함께** 붙어 있었다
> (`早インパラを被ったうさぎ` → `ちいかわキャラクターズ, うさぎ`).
> **작품 단위 태그를 근거로 쓰고, 캐릭터 태그는 `labels`로만 쓴다.**

### 7.2 혼재 상품

`【予約】ナガノキャラクターズ ハッピーバッグ2027` → 태그가 `ナガノのくま, パグ, もぐらコロッケ`뿐이다.
치이카와 태그가 없으므로 **제외**된다.

치이카와가 섞인 럭키백이면 `ちいかわキャラクターズ`가 붙을 것이다. 그 경우 **수록 + `他キャラ混在` 라벨**을 낸다.
...
| 3 | 치이카와 럭키백에 `ちいかわキャラクターズ`가 실제로 붙는지 | 안 붙으면 §7.2를 제목 매칭으로 보강 |
| 4 | 캐릭터 태그 목록의 전체 범위 | `labels` 화이트리스트 확정에 필요 |
| 5 | `RE` 6자리 변종이 `PRE`·발매 태그에도 있는지 | 있으면 모든 태그 규칙을 배열로 |

**해소됨** — もぐもぐ本舗 태그 체계(예약·재입고 **없음**), sitemap 취득 방식(인덱스 필수),
컬렉션 핸들 형식(파싱 금지), 점포 한정 취급(`labels`),
`nagano-market.jp` 컬렉션 패턴(`chiikawamarket.jp`와 동일: `YYYYMMDD` / `re*` / `pre*`),
관련성 필터(§7).
