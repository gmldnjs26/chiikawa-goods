...
>
> [!warning] 픽스처는 이 필드를 아직 갖고 있지 않다 (2026-09-04)
> 픽스처 채집이 확장 **전**의 화이트리스트로 이뤄졌다. 그래서 로컬 `mention` 614건이
> 전부 `images` 없이 저장돼 있고, 정규화하면 `item.image_url`이 614건 모두 `NULL`이다.
> **코드 결함이 아니다** — 입력에 없는 것을 만들어내지 않은 것이고, 그게 맞는 동작이다.
> 재채집하면 채워진다. 카드에 이미지가 보이려면 재채집이 선행한다.
...
| `price` | `min(variants[].price)` | **문자열로 온다** (`"1870"`). JPY 정수로 바꾼다. §2.1 |
...
| `series_total` | 제목의 `（全N種）` | **전각 숫자가 온다** — `（全７種ＢＯＸ）` 실측. 없으면 `NULL` |
...
> [!important] 접두어가 붙은 변종 태그가 실재한다 (실측 2026-09-04, 픽스처)
> 같은 상품에 `20260710`과 `chiikawamovie-20260710` · `eiga-20260710`이 함께 있었고,
> `PRE20260828`과 `chiikawamovie-PRE20260828`도 마찬가지였다.
> → **태그 규칙은 `^`와 `$`로 앵커한다.** 앵커가 없으면 캠페인 접두어가 붙은 변종이
> 같은 날짜를 중복 매칭한다. 시드는 이미 앵커돼 있다.

> [!warning] `ST20260820` — 의미를 모른다. 파싱하지 않는다 (2026-09-04)
> 발매(`20260710`) · 예약(`PRE20260828`) · 재입고(`RE20260825`) 태그를 **전부 가진**
> 상품에 함께 붙어 있었다. `ST`가 무엇의 약자인지 근거가 없다 —
> 販売開始(판매개시)일 수도, 店舗(점포)일 수도, 다른 것일 수도 있다.
>
> **추측으로 파싱하지 않는다.** `config`에 규칙이 없으므로 자동으로 무시되고,
> `labels` 화이트리스트에도 없으므로 화면에도 안 나온다. 근거가 생기면 그때 넣는다.
> ([[plan]] 「없는 정보를 만들지 않는다」)
...
두 스토어 공통으로 **`series_tags`** 를 더한다 (2026-09-12, #20). 태그 리터럴과 일치하면 `item.series`에
**이 배열 순서로** 들어간다 — 첫 원소가 대표 시리즈다. 구체적인 것(`超まじかるちいかわ`)을 일반적인 것(`まじかるちいかわ`)보다
앞에 둔다. 목록은 §9.3의 실측이다.

목록은 §9.3(실측 인벤토리)이고 값의 진실은 `ItemSeries` migration이다 — 여기 복사하지 않는다.
...
> [!important] `mixed_marker_tags`는 **판정 입력이 아니라 라벨 근거**다 (명시 2026-09-04)
> 위 3규칙에 `mixed_marker_tags`가 없다. 그게 의도다.
>
> 이 태그들이 하는 일은 **컬렉션이 순수하지 않다는 증거**다 — 그래서 규칙 2가
> 「컬렉션만 걸리면 `mixed`」인 것이다. 이미 작품 태그가 붙은 상품을
> **끌어내리지 않는다.** §7.2가 그대로 적고 있다 — 「붙으면 `included`,
> 컬렉션만 걸리면 `mixed` + `他キャラ混在` 라벨」.
>
> 틀리기 쉬운 두 방향:
> - 컬렉션만 걸린 것을 `included`로 올린다 → 다른 작품 굿즈가 치이카와로 뜬다
> - 작품 태그가 붙었는데 마커가 함께 있다고 `mixed`로 내린다 → 정상 상품이 경고를 단다
>
> `mixed`는 **수록한다.** `item`으로 승격하고 `他キャラ混在` 라벨을 붙인다.
> 승격 대상은 `included` **+ `mixed`**다.
...
| `raw_payload`를 **줄인다** | 용량. `tags` + `_collections`만 남긴다 |
| `item`으로 승격하지 않는다 | 화면에 안 나온다 |
| 제외 건수를 `collection_run.excluded_count`에 남긴다 | 신규 컬럼 |

> [!warning] 줄인다는 것이 `NULL`이라는 뜻은 아니다
> 복구 경로는 「규칙을 고치고 `excluded` mention을 **재처리**한다」이고,
> 재처리에 필요한 것이 **태그와 컬렉션**이다. `NULL`로 만들면 §7.3의 목적 자체가 사라진다.
> `raw_title`·`url`은 이미 컬럼이므로 payload에 남길 것은 `tags`와 `_collections`다.
>
> 줄인 payload는 `payload_hash`가 달라진다. 규칙이 바뀌면 새 행이 생기는 것이 맞다 —
> **`mention`을 UPDATE하지 않는다.** 과거 행도 소급 수정하지 않는다.

**제외 비율이 급변하면 경보를 낸다.** 평소 70% 제외인데 갑자기 99%면 태그 체계가 바뀐 것이다.
규칙을 고치고 **`relevance='excluded'` mention을 재처리**하면 복구된다.

> [!warning] 재처리 커맨드는 **아직 없다** (2026-09-05)
> 재처리에 필요한 데이터(`tags` · `_collections`)는 남기고 있지만, 저장된 `relevance`를 현재
> `config`로 다시 판정하는 소비자가 없다. `cli normalize`는 저장된 값을 그대로 믿고 건너뛴다.
> 지금 유일하게 살아 있는 복구는 재채집이고, 그건 외부 요청이다. 에픽 F(운용)에서 만든다.

이력이 없으면(첫 실행·전량 `enabled=false`) **경보하지 않는다.** 비교 대상이 없다.
...
### 9.3 시리즈·콜라보 태그 → `item.series` (브랜드가 아니다)

> 2026-09-07: 브랜드는 스토어 브랜드로 확정했다 ([[data-collection-design]] §9.4).
> 2026-09-12: 아래는 `labels`가 아니라 **`series`** 다 — `source.config.series_tags`(§6)로 판정한다 ([[db-schema]] §5.2).
> 실측 2026-09-06 (chiikawamarket 705건): `映画ちいかわ` 258 · `ちいかわパーク` 48 · `まじかるちいかわ` 13 ·
> `超まじかるちいかわ` 8 · `シーサーのおみやげやさん` 4. 두 개 겹침 8건, 없음 382건.
