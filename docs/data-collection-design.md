...
| **1시간** | 공식 스토어 sitemap + preorder | 신규 `pre*` 검출. **30분에서 내렸다** (아래) |
...
> [!note] 공식 스토어 주기를 30분 → **1시간**으로 내렸다 (결정 2026-08-29)
> 이용규약이 「사전 허가 없는 자동 정보 취득」을 금지행위로 적는 상태에서
> 허가를 구하러 갈 예정이다 ([[source-mapping]] §6.2). **묻기 전에 부하부터 절반으로 줄인다.**
> 「30분마다 긁겠다」와 「1시간마다 긁겠다」는 문의서에서 다른 무게를 갖는다.
>
> 잃는 것: 신규 검출이 최대 1시간 늦는다. 예약 사전 예고가 median 3일이므로(§5)
> 예약 감지에는 영향이 없다. **발매·재입고 순간 감지는 어차피 §6의 1분 한정 구간이 맡는다.**
> 재입고는 예고 0일이 기본이라 1시간 주기로는 원래 못 잡는다 — 그건 이 주기의 일이 아니다.
...
| 2 | 시리즈 컬렉션 소속 매칭 |
| 3 | 제목 문자열 규칙 매칭 |
| 4 | **소스 매칭** — `match_rules.sources`에 그 mention의 `source.code`가 있다 (2026-09-07) |
| 5 | 실패 → **`NULL`(미판정)**. 화면에는 `その他` (§9.3 수동 교정) |

> [!important] 브랜드는 **스토어 브랜드**다 (2026-09-07 확정)
> `ちいかわマーケット` `ナガノマーケット` `ちいかわポケット` `もぐもぐ本舗` `一番くじ` — [[plan]] §6.6의 이름표.
> `映画ちいかわ` `ちいかわパーク` 같은 **시리즈는 브랜드가 아니라 `labels`다.** 한 발표(컬렉션) 안에
> 시리즈가 섞이므로(`20260904` 59건 중 ちいかわパーク 36 · 映画 2 · 없음 21) 시리즈로 나누면
> 발표 단위 묶음([[db-schema]] §6)이 갈라진다.
>
> **실측 (2026-09-06, chiikawamarket 705 · mogumogu 606):** 상품 태그 · 컬렉션 · 제목 어디에도
> 스토어명이 없다 — 1~3단계 매칭률 **0%**. 그래서 4단계가 있다. 그 스토어의 상품이라는 사실이 근거다.
> 4단계가 마지막인 이유는 같은 스토어에서 팔리는 `一番くじ` 같은 것을 태그(1단계)로 먼저 가려내기 위해서다.
...
1시간 폴링은 대부분 무변경이라 해시 비교만으로 대부분 줄어든다.
