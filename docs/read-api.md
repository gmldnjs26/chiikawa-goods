...
- **발표 단위 접기는 캘린더만** — API가 사건을 접어 준다 (§4.0). 홈은 카드 그대로다 (§3.4)
...
interface Card {
  ...
  labels: string[];
  series: string[];               // 소속 시리즈 '映画ちいかわ'. 첫 원소가 대표. 없으면 []
...
`drop_group` 행 그대로다. **같은 `drop.id`의 카드는 같은 발표에서 나온 것**이다. 화면이 접는 데 쓰지는 않는다 —
캘린더 접힌 사건의 제목 근거(§4.0)다. `title`이 `null`이면 화면이 브랜드 · 날짜 · `kind`로 이름을 만든다 —
...
섹션 배열은 **카드 1건 = 1원소**이고 화면도 그대로 낸다 (2026-09-12, #20에서 확정). 홈 카드는 이미지 · 뱃지 · 가격 ·
날짜를 각자 갖고 있어 발표로 접으면 그 정보가 숨는다 — 유저는 홈에서 「살 수 있는 물건」을 훑고, 발표 단위는 캘린더(§4.0)가
맡는다. `Card.drop`은 캘린더 접힌 사건의 제목 근거(§4.0)로만 쓴다. 필터(§3.3)도 카드 단위다.
...
**접힌 사건의 표시** (2026-09-12, #20). 접힌 행에는 대표 이미지를 **`items` 순서대로 최대 4장** 스트립으로 낸다
(`imageUrl`이 있는 것만 — 이미지 게이트 §6.2를 통과한 카드). 펼치면 `series[0]`으로 소그룹을 만든다 —
「ちいかわパーク 36」 「映画ちいかわ 2」, 시리즈 없는 것은 「その他」로 **마지막에**. 소그룹 순서는 건수 내림차순.
시리즈는 접기 **키가 아니다** — 키에 넣으면 발표가 갈라진다 ([[db-schema]] §5.2). **캘린더에 한한다** — 홈은 접지 않는다 (§3.4).
